...
function addSpecialEffects(tilesToDelete){
	if (tilesToDelete.length < 3) return tilesToDelete;
	var allTilesToDelete = [...tilesToDelete];
		...
		switch (type) {
			case 1: //red
				...
				neighbours.forEach(offset => {
					var newX = x + offset.x;
					var newY = y + offset.y;
					if (isCorrectTile(newX, newY) && allTilesToDelete.filter(tile => tile.x === newX && tile.y === newY).length === 0) {
						allTilesToDelete.push({x: newX, y: newY});
					}
				});
				break;
			case 2: // green
				var neighbours = [{x:-1, y: 0}, {x:1, y: 0}, {x:0, y:-1}, {x:0, y:1}];
				neighbours.forEach(offset => {
					var newX = x + offset.x;
					var newY = y + offset.y;
					if (isCorrectTile(newX, newY) && allTilesToDelete.filter(tile => tile.x === newX && tile.y === newY).length === 0) {
			...
			case 4: // yellow
				var neighbours = [{x:-1, y:0}, {x:1, y:0}, {x:0, y:-1}, {x:0, y:1}];
				var tilesToCheck = [].concat(tilesToDelete);
				while (tilesToCheck.length > 0) {
					var current = tilesToCheck.pop();
					neighbours.forEach(offset => {
						var newX = current.x + offset.x;
						var newY = current.y + offset.y;
						console.log(newX, newY);
						if (isCorrectTile(newX, newY) && allTilesToDelete.filter(tile => tile.x === newX && tile.y === newY).length === 0 && tiles[newX][newY] === 4) {
							allTilesToDelete.push({x: newX, y: newY});
							tilesToCheck.push({x: newX, y: newY});
						}
					})
				}
				break;
